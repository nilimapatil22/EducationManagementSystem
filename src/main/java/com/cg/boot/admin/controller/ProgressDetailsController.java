...
public class ProgressDetailsController {
	 ...
	 * @throws DataNotFoundException
	 */
	@RequestMapping(method=RequestMethod.PUT ,path="/updateGrade/{gradeId}")
	public ResponseEntity<ProgressDetails> updateProgressDetails(@PathVariable ("gradeId")int gradeId, @RequestBody ProgressDetails progressDetails)
			throws DataNotFoundException {
		ProgressDetails gradeInfo = service.updateProgressDetails(progressDetails,gradeId);
		if (gradeInfo == null) {
	 ...
	 * @return {@link ResponseEntity} grades {@link List} {@link HttpStatus}
	 */
	@DeleteMapping("/deleteProgressDetails/{progressId}")
	public ResponseEntity<List<ProgressDetails>> deleteProgressDetails(@PathVariable("progressId") int progressId) {
		List<ProgressDetails> grades = service.deleteProgressDetails(progressId);
		logger.warn("Progress Details with progress ID "+progressId+" Deleted successfully");
		return new ResponseEntity<List<ProgressDetails>>(grades, HttpStatus.OK);
	}
