...
class ProgressDetailsServiceTest {
	...
	public void getAllProgressDetailsTest() {
		...
		when(repository.findAll()).thenReturn(Stream.of(new ProgressDetails(2, "A", "2021-02-06",2, 9, 31)).collect(Collectors.toList()));
		assertEquals(1,service.getAllProgressDetails().size());
		}
	
	@Test
	public void updateProgressDetailsTest() {
		ProgressDetails details=new ProgressDetails(2, "A", "2021-02-06",2, 9, 31);

		when(repository.findAll())
				.thenReturn(Stream.of(new ProgressDetails(2, "A", "2021-02-06",2, 3, 4)).collect(Collectors.toList()));
		assertEquals(1, service.getAllProgressDetails().size());
	}
	...
	public void addProgressDetailsTest() {
		ProgressDetails details= new ProgressDetails(2, "A", "2021-02-06",2, 37, 31);
	}
